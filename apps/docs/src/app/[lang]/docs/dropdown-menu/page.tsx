import { MenuDemo } from "@/components/interactive";
...
export async function generateMetadata({ params }: { params: Promise<{ lang: Locale }> }) {
  ...
  return { title: findPage("dropdown-menu")!.title[lang] };
}
...
const T = {
  tr: {
    lead: "Eylem listesi, ve tetikleyicisine yapışır; nereden geldiği hiç belirsiz kalmaz.",
    danger: (
      <>
        Yıkıcı eylem <code>state=&quot;danger&quot;</code> alır ve ayracın altına iner; bir liste
        içinde kırmızı bir satır, komşusuna yanlışlıkla basılmasını engelleyen tek şeydir.
      </>
    ),
    align: (
      <>
  ...
  en: {
    lead: "An action list, stuck to its trigger, where it came from is never in doubt.",
    danger: (
      <>
        A destructive action takes <code>state=&quot;danger&quot;</code> and drops below the
        separator: inside a list, a red row is the only thing that stops someone hitting its
...
export default async function Page({ params }: { params: Promise<{ lang: Locale }> }) {
  ...
  const p = findPage("dropdown-menu")!;
  const t = T[lang];
  return (
    <>
      <PageHead title={p.title[lang]} blurb={p.blurb[lang]} />
      <P>{t.lead}</P>
      <Demo labels={dict.demo} code={`<DropdownMenu
  trigger={<button className="tamga-icon-btn"><Icon icon={More} /></button>}
  items={[
    { label: "…", onSelect: refresh },
    { label: "…", onSelect: exportAll },
    { kind: "separator" },
    { label: "…", state: "danger", onSelect: remove },
  ]}
/>`}>
        <MenuDemo lang={lang} />
      </Demo>
      ...
      <H2>Props</H2>
      <Props of="DropdownMenu" lang={lang} />
    </>
  );
}
